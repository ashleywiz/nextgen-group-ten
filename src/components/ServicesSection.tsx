...
const ServicesSection = () => {
  const services = [{
    ...
    description: "We support contractors and civil projects with safe clearing of post-demolition debris. Our demolition waste disposal in Goa coordinates with your site team so that concrete, bricks, and structural waste move out quickly and safely.",
    idealFor: ["Construction companies", "Contractors", "Civil projects"]
  }];
  return <section id="services" className="py-20 md:py-28 bg-secondary/50">
      <div className="container mx-auto">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-secondary/50 rounded-xl p-6">
                ...
                <ul className="space-y-2">
                  {services[0].collects?.map((item, idx) => <li key={idx} className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              <div className="bg-destructive/5 rounded-xl p-6">
                <h4 className="font-semibold text-foreground mb-4 flex items-center gap-2">What We Don't Collect (from the General Public)<XCircle className="w-5 h-5 text-destructive" />
                  What We Don't Collect
                </h4>
                <ul className="space-y-2">
                  {services[0].notCollects?.map((item, idx) => <li key={idx} className="flex items-center gap-2 text-sm text-muted-foreground">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {services.slice(1).map((service, index) => <div key={index} className="bg-card rounded-2xl p-6 eco-shadow-md border border-border hover:border-primary/30 transition-all hover:-translate-y-1">
              ...
              {service.idealFor && <div className="flex flex-wrap gap-2">
                  {service.idealFor.map((item, idx) => <span key={idx} className="text-xs bg-secondary px-3 py-1 rounded-full text-secondary-foreground">
                      {item}
                    </span>)}
                </div>}
            </div>)}
